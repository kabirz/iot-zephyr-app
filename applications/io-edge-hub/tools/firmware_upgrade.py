...
def upgrade_udp(ip: str, port: int, image_path: str, test: bool,
    ...
    try:
        # 1. FW_START
        progress.message(f"[1/4] FW_START (擦写 slot1, 请稍候 ~5s)...")
...
def upgrade_can(channel: str, image_path: str, test: bool,
    ...
    try:
        # 1. 发 keyhash (可选)
        if img.keyhash:
            progress.message(f"[1/4] 发送 keyhash (5 帧 0x104)...")
        ...
        mode = "测试模式 (重启后回滚)" if test else "永久模式"
        progress.message(f"[4/4] CONFIRM OK ({mode}), 设备下次启动 MCUboot 交换")
    finally:
        up.close()
