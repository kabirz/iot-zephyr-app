 ...
 * (移植自 io-edge-hub src/modbus/tcp.c, 原样保留)
 *
 ...
 *   - 会话超时 (CANOPEN_IO_MODBUS_TCP_SESSION_TIMEOUT, 默认关闭)
 ...
 * 由 CONFIG_CANOPEN_IO_MODBUS_TCP_SESSION_TIMEOUT 使能 (默认 n, 不踢空闲连接)。 */
...
#ifdef CONFIG_CANOPEN_IO_MODBUS_TCP_SESSION_TIMEOUT
...
K_THREAD_DEFINE(mb_tcp, CONFIG_CANOPEN_IO_MODBUS_TCP_STACK, mb_tcp_thread, NULL, NULL, NULL,
		CONFIG_CANOPEN_IO_MODBUS_TCP_PRIORITY, 0, 0);
