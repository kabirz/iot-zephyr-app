 ...
 * (移植自 io-edge-hub src/modbus/function.c, 去除 CAN 业务寄存器;
 *  DO 寄存器与 CANopen OD 0x2002 双向同步)
 *
...
/* ==================== 寄存器数组 (唯一数据源) ==================== */
/* 采样使能/间隔默认值沿用原 OD 0x2004 出厂默认 ({0xFFFF,0x000F,100,100}) */
static uint16_t holding_reg[CONFIG_MODBUS_HOLDING_REGISTER_NUMBERS] = {
	...
	[HOLDING_DI_SAMPLE_MS_IDX] = 100,    /* DI 采样间隔 ms */
	[HOLDING_AI_SAMPLE_MS_IDX] = 100,    /* AI 采样间隔 ms */
 ...
 * (0x0C/0x0D) 返回实时系统时间, 其余返回数组值。供 Web /api/regs
...
/* 触发全量保存 (供 OD 0x2004:5 / Web / UDP handler 改参数后持久化)。
 ...
 * 供 Modbus 写回调与 Web (HTTP/WS)/UDP 共用, 保证所有写入路径行为一致。 */
int io_write_holding(uint16_t addr, uint16_t reg)
{
	...
	switch (addr) {
	case HOLDING_DO_IDX:
		/* DO 输出 + LED 联动 + OD 0x2002 回读 (TPDO 触发在 mb_set_do 内) */
		mb_set_do(reg & 0xFF);
		break;
...
/* 单 DO 位写 (加锁读-改-写, 与 Modbus FC05 语义一致), 供 Web (HTTP/WS)/Shell 共用 */
...
/* Modbus 用户回调表 (tcp.c/rtu.c 注册给 modbus_init_server) */
