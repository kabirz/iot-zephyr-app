 ...
 * (移植自 io-edge-hub src/web/httpd.c, 去除 CAN 配置字段;
 *  HTTP 服务名 canopen_web, 与 sections-rom.ld 的 iterable section 对应)
 *
 ...
 * reg → io_write_holding (FC06 同路径, 0x0F 重启改为延迟重启保住响应) */
...
int web_cmd_exec_reg(int32_t addr, int32_t value)
{
	...
	if (addr == HOLDING_REBOOT_IDX) {
		/* 立即重启会掐断 HTTP/WS 响应, 改走延迟重启 (housekeeping 刷日志后复位) */
		if (value) {
...
static uint16_t io_web_port = CONFIG_CANOPEN_IO_WEB_PORT;

HTTP_SERVICE_DEFINE(canopen_web, NULL, &io_web_port, CONFIG_HTTP_SERVER_MAX_CLIENTS, 10, NULL,
		    NULL, NULL);

HTTP_RESOURCE_DEFINE(res_index, canopen_web, "/", &index_html_detail);
HTTP_RESOURCE_DEFINE(res_info, canopen_web, "/api/info", &info);
HTTP_RESOURCE_DEFINE(res_io, canopen_web, "/api/io", &io);
HTTP_RESOURCE_DEFINE(res_regs, canopen_web, "/api/regs", &regs);
HTTP_RESOURCE_DEFINE(res_do, canopen_web, "/api/do", &api_do);
HTTP_RESOURCE_DEFINE(res_reg, canopen_web, "/api/reg", &reg);
HTTP_RESOURCE_DEFINE(res_time, canopen_web, "/api/time", &api_time);
HTTP_RESOURCE_DEFINE(res_cfg, canopen_web, "/api/cfg", &cfg);
HTTP_RESOURCE_DEFINE(res_save, canopen_web, "/api/save", &save);
HTTP_RESOURCE_DEFINE(res_reboot, canopen_web, "/api/reboot", &reboot);
HTTP_RESOURCE_DEFINE(res_history, canopen_web, "/api/history", &history);
HTTP_RESOURCE_DEFINE(res_download, canopen_web, "/api/history/download", &download);
HTTP_RESOURCE_DEFINE(res_hist_del, canopen_web, "/api/history/delete", &hist_del);
HTTP_RESOURCE_DEFINE(res_ws, canopen_web, "/ws", &ws_io_detail);
