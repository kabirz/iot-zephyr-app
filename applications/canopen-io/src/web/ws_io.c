 ...
 * (移植自 io-edge-hub src/web/ws_io.c, 升级通道锁改为 UDP/WS/SDO 三通道)
 *
 ...
 *   {"cmd":"time","ts":unix}
 *   {"cmd":"cfg","ip":"a.b.c.d","rs485":n,"sid":n}
 *   {"cmd":"save"}	参数持久化到 FCB
...
int ws_io_build_status(char *buf, size_t bufsz)
{
	...
	n += snprintf(buf + n, bufsz - n, "],\"di_en\":%u,\"ai_en\":%u,\"ms\":%lld}", di_en,
		      ai_en, (long long)k_uptime_get());
...
static K_THREAD_STACK_ARRAY_DEFINE(ws_stacks, CONFIG_CANOPEN_IO_WEB_WS_HANDLERS,
				   CONFIG_CANOPEN_IO_WEB_WS_STACK_SIZE);
static struct ws_slot ws_slots[CONFIG_CANOPEN_IO_WEB_WS_HANDLERS];

static int ws_get_free_slot(void)
{
	for (int i = 0; i < CONFIG_CANOPEN_IO_WEB_WS_HANDLERS; i++) {
 ...
 * (与 UDP 升级通道的 keyhash 预校验对齐; WS 无独立 keyhash 帧,
...
static void ws_handle_cmd(struct ws_slot *s, const char *cmd, size_t len)
{
	...
	if (strncmp(cmd, "\"do\"", 4) == 0 && json_get_i32(cmd, len, "index", &index) &&
		...
		if (active) {
			...
			if (json_get_str(cmd, len, "keyhash", b64_keyhash,
					 sizeof(b64_keyhash))) {
			...
			if (!fw_upg_verify_client_keyhash(keyhash_ptr, keyhash_len)) {
				...
				if (flash_area_open(SLOT1_PARTITION_ID, &fa) != 0) {
					...
					if (rc != 0 || flash_img_init(&fw_upg.fic) != 0) {
						...
						LOG_INF("fw upgrade started (size=%d)",
							fw_size);
						n = snprintf(s->tx_buf, sizeof(s->tx_buf),
							     "{\"ok\":true}");
					}
		...
		if (!active) {
			...
			if (received == 0) {
				...
				n = snprintf(s->tx_buf, sizeof(s->tx_buf),
					     "{\"ok\":false,\"err\":\"keyhash mismatch\"}");
			} else if (boot_request_upgrade(BOOT_UPGRADE_PERMANENT) != 0) {
				n = snprintf(s->tx_buf, sizeof(s->tx_buf),
			...
			if (type & WEBSOCKET_FLAG_TEXT) {
				...
				if (active) {
					if (flash_img_buffered_write(
						    &fw_upg.fic, (const uint8_t *)s->rx_buf, len,
						    false) == 0) {
						k_mutex_lock(&fw_upg_lock, K_FOREVER);
						fw_upg.crc = crc16_ccitt(
							fw_upg.crc, (const uint8_t *)s->rx_buf,
							len);
						fw_upg.received += len;
						k_mutex_unlock(&fw_upg_lock);
					} else {
...
SYS_INIT(ws_slots_init, APPLICATION, CONFIG_CANOPEN_IO_INIT_PRIORITY_WS);
