 ...
 *
 * 应用调试 shell (根命令 io)
 * (移植自 io-edge-hub src/shell.c, 去除 CAN 配置子命令)
 *
 *   io                     -- 查看 IO/配置总览
 *   io info                -- 版本/MAC/IP/链路/RS485/节点号 基本信息
 *   io di                  -- DI1-16 状态
...
static int cmd_info(const struct shell *sh, size_t argc, char *argv[])
{
	...
	shell_print(sh, "node-id : %d (CANopen, heartbeat via OD 0x1017)", CONFIG_CANOPEN_NODE_ID);
...
static int cmd_ip(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (sscanf(argv[1], "%u.%u.%u.%u", &a, &b, &c, &d) != 4 || a > 255 || b > 255 ||
	    c > 255 || d > 255 ||
 ...
 * (housekeeping history_sync + 排空日志后冷重启, 与 UDP/Web 路径一致) */
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_io, SHELL_CMD(info, NULL, "show version / mac / ip / node / rs485 info", cmd_info),
...
SHELL_CMD_REGISTER(io, &sub_io, "canopen-io debug commands", NULL);
