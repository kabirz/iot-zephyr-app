 ...
 * (移植自 io-edge-hub src/bwtest.c, 原样保留)
 *
 *  - 监听 CONFIG_CANOPEN_IO_BW_TEST_PORT (默认 9900)
 *  - 单连接: 收多少回显多少 (echo), 用于测量协议栈原始 TCP 吞吐
 *  - Kconfig 开关 CONFIG_CANOPEN_IO_BW_TEST, 测完即移除, 不影响正式固件
...
static void bw_echo_thread(void *p1, void *p2, void *p3)
{
	...
	struct sockaddr_in addr = {
		...
		.sin_port = htons(CONFIG_CANOPEN_IO_BW_TEST_PORT),
	};
	...
	LOG_INF("BW test echo server on port %d", CONFIG_CANOPEN_IO_BW_TEST_PORT);
