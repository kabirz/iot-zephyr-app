			...
			if (FD_ISSET(serv, &rfds)) {
				...
				if (c >= 0) {
					...
					if (slot < 0) {
						...
						ftp_send(c, "220 canopen-io FTP service ready");
					}
				}
			}
			for (int i = 0; i < FTP_MAX_CLIENTS; i++) {
...
K_THREAD_DEFINE(ftp, CONFIG_CANOPEN_IO_FTP_STACK_SIZE, ftp_thread, NULL, NULL, NULL, CONFIG_CANOPEN_IO_FTP_PRIORITY,
		0, 0);
