 ...
 *
 * canopen-io 公共定义: IO 通道数、Modbus 寄存器枚举、历史数据结构、
 * 全局访问函数声明。(移植自 io-edge-hub include/init.h, 去除 CAN 业务寄存器)
 *
 * holding_reg[] / input_reg[] 是唯一的参数与采样数据源:
 *   - settings (FCB) 直接映射 holding_reg[] (modbus/ 命名空间)
 *   - DI/AI 采样线程写入 input_reg[] 并镜像到 OD 0x2001 / 0x2000
 *   - DO 写 holding_reg[0x00] 经回调驱动 GPIO + LED + OD 0x2002 回读
 */
...
/* ==================== Holding Registers (读写, 16 个) ==================== */
enum holding_reg_idx {
	HOLDING_DO_IDX = 0x00,      /* DO1-8 输出控制 (与 OD 0x2002 同步) */
	...
	HOLDING_RS485_BAUDRATE_IDX, /* 0x06 RS485 波特率 */
	HOLDING_SLAVE_ID_IDX,       /* 0x07 Modbus RTU Slave ID */
	HOLDING_IP_OCTET1_IDX,      /* 0x08 IP 段1 */
	HOLDING_IP_OCTET2_IDX,      /* 0x09 IP 段2 */
	HOLDING_IP_OCTET3_IDX,      /* 0x0A IP 段3 */
	HOLDING_IP_OCTET4_IDX,      /* 0x0B IP 段4 */
	HOLDING_TIMESTAMP_HI_IDX,   /* 0x0C 时间戳高16位 */
	HOLDING_TIMESTAMP_LO_IDX,   /* 0x0D 时间戳低16位 */
	HOLDING_CONFIG_SAVE_IDX,    /* 0x0E 参数保存触发 */
	HOLDING_REBOOT_IDX,         /* 0x0F 写1触发重启 */
};
...
/* 出厂恢复: 擦除 settings 分区并重启加载默认值 */
int settings_factory_reset(void);

/* ==================== DO 控制 (dio.c) ==================== */
/* 设置 DO 输出 + LED 联动 + OD 0x2002 回读/TPDO 触发,
 * val bit0-7 对应 DO1-DO8 (不经 holding_reg, 调用方已更新寄存器) */
int mb_set_do(uint16_t val);

/* ==================== 时间管理 (time.c) ==================== */
/* 设置 RTC + 系统时钟 (Modbus 0x0C/0x0D 或 UDP/Web 命令调用).
...
/* ==================== 延迟重启 (main.c) ==================== */
/* 设置延迟重启标志 (housekeeping 刷日志后重启), 供 UDP/Web 改 IP 等调用 */
