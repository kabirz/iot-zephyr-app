...
**iot-zephyr-app** — Zephyr RTOS west manifest 仓库，同时本身是一个 Zephyr module。包含两个运行于 STM32F103RCT6（板 `nrf24_f103rct6`）的配套应用：

- `applications/angle-handler` — 激光测距手持控制器（角度采集 + OLED + CAN/nRF24 + OTA）
- `applications/n2e-gw` — 数据中转网关（nRF24 ↔ W5500 UDP）
...
- `boards/` — board_root，目前仅 `nrf24_f103rct6`。
...
- **MCUboot 签名**：每个 app 自带 `boards/nrf24_f103rct6.pem`（RSA-2048），在 `sysbuild.conf` 用 `${APP_DIR}/boards/nrf24_f103rct6.pem` 引用。两 app 密钥独立、镜像互不通用。
...
- **CONFIG 符号前缀**：angle-handler 用 `ANGLE_HANDLER_*`，n2e-gw 用 `N2E_GW_*`。
...
- CMake `project()` / C 标识符 / CONFIG 符号用下划线形：`angle_handler`、`n2e_gw`（连字符在 C/CMake 变量中非法）。
